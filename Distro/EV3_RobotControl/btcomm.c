...
#define __BT_debug  // Uncomment to trigger printing of BT messages for debug
                    // purposes
...
int BT_motor_port_start(char port_ids, char power) {
  ...
  unsigned char *cp;
  char reply[1024];
  unsigned char cmd_string[15] = {0x0D, 0x00, 0x00, 0x00, 0x80,
...
int BT_turn(char lport, char lpower, char rport, char rpower) {
  ...
  //gently 	    BT_turn(MOTOR_A, 100, MOTOR_B, 50);      <-- Turn toward the left
  //more sharply 	    BT_turn(MOTOR_A, 100, MOTOR_B, 0);       <-- Turn toward the
  //left at the highest possible rate
  ...
  //at full speed 	    BT_turn(MOTOR_A, -50, MOTOR_B, 50);      <-- Spin clockwise at
  //half speed
  //
  // Inputs: port identifier of left port
